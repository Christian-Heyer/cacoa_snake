...
if (exists("snakemake")) {
    ...
    cao_output <- file.path(base_fp, "processed_cao.RDS.gz")
      ...
      aged = "#A0A0A4")
      plot_dir <- "/omics/odcf/analysis/OE0228_projects/VascularAging/rna_sequencing/public_scrnaseq/LungAtlas/plots"
...
plot_gene_bar <- function(deseq_obj, gene_id, cell_type) {
  gene_id<- ifelse(org_name == "hs", str_to_upper(gene_id), gene_id)
...
for(c_g in cell_types) {
  ...
  plots <- list()
  if ("Aplnr" %in% rownames(yeet) |"APLNR" %in% rownames(yeet) ) {
    plots[["aplnr"]] <- plot_gene_bar(yeet, "Aplnr", c_g)
  }
  if("Apln" %in% rownames(yeet) | "APLN" %in% rownames(yeet) ) {
    plots[["apln"]] <- plot_gene_bar(yeet, "Apln", c_g) 
  }
  if("Cd74" %in% rownames(yeet) | "CD74" %in% rownames(yeet)) {
...
calculate_pc_expression <- function(group_id, group_name, seurat_obj, feature = "Aplnr") {
  feature <- ifelse(org_name == "hs", str_to_upper(feature), feature)
...
sample$cell <- NULL
p <- SCpubr::do_BoxPlot(sample = sample[,sample@meta.data$ann_level_1 == "Endothelial"],
feature = "APLNR", group.by = "binary_age",assay = "RNA")#,use_test = T,comparisons = list(c("aged","young")))
...
pct_expression <- ggplot(pct_expression_data, aes(x = id, y = pct.exp, fill = age)) + geom_col() +
  scale_x_discrete(guide = guide_axis(angle = 90)) + scale_fill_manual(values = c("aged" = "#A0A0A4",
                                                                                  "young" = "#FC8D59"),
                                                                       labels = c("aged" = "aged",
                                                                                  "young"= "young"),
                                                                       breaks=c("young", "aged")) +
...
```{r, fig.width =10}
#cell_type_markers
marker_genes <- c("Ager", "Apln", "Aplnr" ,"Calca", "Ccr2", "Cd14", "Cd19", "Cd200r3", "Cd24a", "Cd3e", "Cd68", "Cd79a", "Cd8a", "Col1a1", "Csf1r", "Cx3cr1", "Cxcr4", "Epcam", "Foxj1",
                  "H2-Aa", "Itgam", "Itgax", "Klrb1c", "Krt5", "Ly6c2", "Marco", "Mrc1", "Pdpn", "Pecam1", "Ptprc", "Resp18", "Scgb1a1", "Scgb3a2", "Sftpb", "Wnt3a")

DefaultAssay(object = seurat_obj)  <- "RNA"
p <- SCpubr::do_DotPlot(seurat_obj,features = marker_genes, group.by = "cell_ontology_class", cluster = TRUE)
ggsave("paper_rev/markergenes.svg",p,width = 10)
p
```
...
```{r}
pray_to_jesus <- readRDS("/omics/groups/OE0436/internal/heyer/glmmseq_result.RDS.gz")

eval_seq <- function(glmmseq_obj) {
    stats <- summary(glmmseq_obj)+
    cat("\n \n")
    cat("### Pvalues \n")
 #   hist(glmmseq_obj@stats$pvals[,])
   # apply(glmmseq_obj@stats$pvals, 2, hist) 
    
    cat("\n \n")
    cat("### glmmseq Qval Res \n")
    glmmseq_obj<- glmmSeq::glmmQvals(glmmseq_obj,cutoff = 0.01)

  #  hist(glmmseq_obj@stats$qvals[,])
    x <- apply(glmmseq_obj@stats$pvals,2, p.adjust, method = "BH")
    
   # hist(x)
    glmmseq_obj
}
glmmseq_norm_counts <- eval_seq(pray_to_jesus$norm_counts)
groups_to_test <- c("age" = "ageaged",
                        "EC_status" = "EC_statustumor",
                        "Aplnr_KO" = "Aplnr_KOKO",
                        "Apln_treatment" = "Apln_treatmentup",
                        "age:EC_status" = "ageaged:EC_statustumor",
                        "EC_status:Aplnr_KO" = "EC_statustumor:Aplnr_KOKO",
                        "age:Apln_treatment" = "ageaged:Apln_treatmentup")
