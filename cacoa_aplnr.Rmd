...
all_markers<- FindMarkers(
    seurat_obj,
    ident.1 = "endothelial cell", only.pos = TRUE ,assay = "SCT", min.pct = 0.5
)
...
mat<- as.matrix(seurat_obj@assays$SCT@data)

acts <- run_mlm(mat=mat, net=msigdb |> dplyr::filter(collection == "hallmark"), .source='geneset', .target='genesymbol',
                minsize = 5)
acts
```