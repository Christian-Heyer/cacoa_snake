...
for(i in 1:length(top_enrich_go)){
  print(top_enrich_go[[i]][, c('Description', 'GeneRatio', 'p.adjust')])
  cat('\n')
  p <- dotplot(ego_result_signed[[top_enrich_clusters[i]]]) + ggtitle(paste0("signature ",top_enrich_clusters[i] )) 
  print(p)  
}
```
...
# Assuming 'sample' is a column in 'adata' data frame
adata_small$sample <- forcats::fct_relevel(adata_small$sample, "basal", "Hour6")
```
...
#net <- msigdbr::msigdbr(species = "Mus musculus", category = "H") %>% dplyr::select(gs_name, gene_symbol)
...
aplnr_tb %>% arrange(aplnr_cor) %>% head(30)
```
...
```{r}
ego_result_msig_signed <- furrr::future_map(module_list_signed, function(i){
  enricher(gene = i,
         TERM2GENE = msigdb_tb,
         pvalueCutoff = 0.05,
         pAdjustMethod = "BH",
...
ego_result_msig_unsigned <- furrr::future_map(module_list_unsigned, function(i){
  enricher(gene = i,
         TERM2GENE = msigdb_tb,
         pvalueCutoff = 0.05,
         pAdjustMethod = "BH",
...
ego_result_msig_signed[aplnr_res] 

if (nrow(ego_result_msig_signed[[which(aplnr_res)]]@result ) > 0) {
  clusterProfiler::dotplot(ego_result_msig_signed[[which(aplnr_res)]])
}
...
module_list_unsigned[aplnr_res] 

if (nrow(ego_result_msig_unsigned[[which(aplnr_res)]] ) > 0) {
  clusterProfiler::dotplot(ego_result_msig_unsigned[[which(aplnr_res)]])
}
...
yoted <- decoupleR::run_fgsea(as.matrix(aplnr_cor), distinct(msigdb_tb)) %>% dplyr::filter(statistic == "norm_fgsea") %>%
  dplyr::mutate(padjust = p.adjust(p_value, method = "BH")) %>% dplyr::filter(padjust < 0.05)
...
pheatmap::pheatmap(mat_consensus[order(mat_consensus, decreasing = T),,drop =F], cluster_rows = F, cluster_cols = F, color = colorRampPalette(c("blue", "white", "red"))(100))
```
...
```{r}
msigdb_tb %>% dplyr::filter(source == "HALLMARK_OXIDATIVE_PHOSPHORYLATION") %>% mutate(presence = target %in% aplnr_tb$`names(aplnr_cor)`) %>% pull(presence) %>% table(dnn="HALLMARK_OXIDATIVE_PHOSPHORYLATION")
msigdb_tb %>% dplyr::filter(source == "HALLMARK_CHOLESTEROL_HOMEOSTASIS") %>% mutate(presence = target %in% aplnr_tb$`names(aplnr_cor)`) %>% pull(presence) %>% table(dnn="HALLMARK_CHOLESTEROL_HOMEOSTASIS")

msigdb_tb %>% dplyr::filter(source == "HALLMARK_FATTY_ACID_METABOLISM") %>% mutate(presence = target %in% aplnr_tb$`names(aplnr_cor)`) %>% pull(presence) %>% table(dnn=c("HALLMARK_FATTY_ACID_METABOLISM"))
