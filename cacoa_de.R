  ...
  extractRawCountMatrices.Seurat <- function(object, transposed = TRUE) {
    cms <- object$sample.per.cell %>%
      ...
      lapply(function(cids)
        Seurat::GetAssayData(object, layer = "counts")[, cids])
    if (transposed) {
      cms <- lapply(cms, Matrix::t)
    }
    cms
  }
  extractJointCountMatrix.Seurat <- function(object,
                                             raw = TRUE,
                                             transposed = TRUE,
                                             sparse = TRUE) {
    ...
    if (is.null(slot) || slot == "scale.data") {
      dat <- Seurat::GetAssayData(object,
                                  slot = "scale.data",
                                  assay = object@misc$assay.name)
      dims <- dim(dat)
...
if (is_raw) {
  if ("SCT_snn" %in% names(cao$data.object@graphs)) {
    cao$data.object@misc$graph.name <- "SCT_snn"
    cao$data.object@misc$assay.name <- "SCT"
  } else {
    cao$data.object@misc$graph.name <- "RNA_snn"
    cao$data.object@misc$assay.name <- "RNA"
  }
} else {
  cao$data.object@misc$graph.name <- "RNA_snn"
}
...
# cao$estimateDiffCellDensity(type='permutation', verbose=FALSE, name='cell.density.kde')oo
#cao$data.object@assays[[cao$data.object@misc$assay.name]] <- as.A
...
#cao$estimateClusterFreeDE(verbose = T, )
#exc.genes <- cao$test.results$cluster.free.z %>%
#  colnames() %>%
#  .[grepl("Mt-", x = ., ignore.case = TRUE)] %>%
#  c("Malat1")
...
estimateAllStabs <- function(cao_obj, de_n, org_db = org.Mm.eg.db::org.Mm.eg.db) {
  cao_obj$estimateDEStabilityPerGene(de.name = de_n, top.n.genes = 500)
#  cao_obj$estimateOntology(
#    type = "GSEA",
#    org.db = org_db,
#    verbose = T,
#    n.cores = 1,
...
for (de_n in list("de.Wald",  "de.loo")) {
  cao <- estimateAllStabs(de_n = de_n,
                          cao_obj = cao,
                          org_db = org_db)
}
...
extract_res <- purrr::map(cao$test.results$de.loo, function(x)
  x$res)
